...
// Add edit state right after newMedicamento state
code = code.replace(
...
const oldMapRegex = /<div className="flex flex-wrap gap-2 max-h-60 overflow-y-auto p-2 bg-white rounded-lg border border-slate-200">\s*\{medicamentos\.map\(\(med, idx\) => \{\s*const nome = typeof med === 'string' \? med : med\.nome;\s*const custo = typeof med === 'string' \? undefined : med\.custoPorKg;\s*return \(\s*<span key=\{nome \+ idx\} className="inline-flex items-center gap-1 px-3 py-1 bg-emerald-50 text-emerald-800 rounded-full text-xs font-medium border border-emerald-200 shadow-sm">\s*\{nome\} \{custo \? \` \(R\$ \$\{custo\.toFixed\(2\)\}\)\` : ''\}\s*\{isMaster && \(\s*<button onClick=\{\(\) => removeMedicamento\(nome\)\} className="hover:text-red-500 ml-1 text-slate-400 transition-colors">\s*<X className="w-3 h-3" \/>\s*<\/button>\s*\)\}\s*<\/span>\s*\);\s*\}\)\}\s*\{medicamentos\.length === 0 && <span className="text-sm text-slate-400 italic">Nenhum medicamento na lista\.<\/span>\}\s*<\/div>/s;
