...
export function ImportData({ onImportComplete }: ImportDataProps) {
  ...
  useEffect(() => {
    try {
      const historyStr = safeStorage.getItem('IMPORT_ERRORS_HISTORY');
      if (historyStr) {
  ...
  const saveToErrorHistory = (newErrors: string[]) => {
    ...
    try {
      safeStorage.setItem('IMPORT_ERRORS_HISTORY', JSON.stringify(updatedHistory));
    } catch (e) {
  ...
  const clearErrorHistory = () => {
    setErrorHistory([]);
    safeStorage.removeItem('IMPORT_ERRORS_HISTORY');
  };
