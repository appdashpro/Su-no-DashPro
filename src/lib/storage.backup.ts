...
export const storage = {
  syncFromSupabase: async () => {
    try {
      ...
      try {
        ...
        if (delIntQueue.length > 0) safeStorage.removeItem(OFFLINE_DELETE_INTEGRADO_QUEUE);
        ...
        if (safeStorage.getItem(OFFLINE_QUEUE_KEY) && JSON.parse(safeStorage.getItem(OFFLINE_QUEUE_KEY) || '[]').length > 0) {
      ...
      if (allData.length === 0) {
        safeStorage.setItem(INTEGRADOS_KEY, JSON.stringify([]));
        safeStorage.setItem(VISITS_KEY, JSON.stringify([]));
        safeStorage.setItem('LAST_SYNC_TIME', new Date().toISOString());
        safeStorage.setItem('LAST_SYNC_USER', session?.user?.email || 'Usuário logado');
        window.dispatchEvent(new Event('sync-completed'));
        return true;
      }
  ...
  saveVisits: async (visits: Visit[], visitsToSyncToSupabase?: Visit[]): Promise<Visit[]> => {
    ...
    try {
      if (!session || session.user?.id === 'offline') {
        ...
        safeStorage.setItem('LAST_WRITE_TIME', Date.now().toString());
  ...
  deleteIntegrado: async (id: string, visitIds?: string[]) => {
    ...
    safeStorage.setItem(INTEGRADOS_KEY, JSON.stringify(remaining));
    ...
    try {
      ...
      if (!session || session.user?.id === 'offline') {
        ...
        const upsertQ = JSON.parse(safeStorage.getItem(OFFLINE_QUEUE_KEY) || '[]');
        const remainingUpserts = upsertQ.filter((v: any) => v.integradoId !== id);
        safeStorage.setItem(OFFLINE_QUEUE_KEY, JSON.stringify(remainingUpserts));
      } else {
