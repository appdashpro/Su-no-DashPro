...
async function run() {
  ...
  try {
    const ints = await client.query(`SELECT DISTINCT "Integrado" FROM public.registros WHERE "Integrado" ILIKE '%eco%' OR "Integrado" ILIKE '%trajan%' OR "Integrado" ILIKE '%cotra%'`);
