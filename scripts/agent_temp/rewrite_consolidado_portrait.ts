...
export const getConsolidatedLotesTemplate = (
  ...
  if (selectedLotes.length > 0) {
    selectedLotes.forEach((l, index) => {
      ...
      const mortColor = mortPercentNum > 3.0 ? '#ef4444' : '#334155'; // Red if > 3%
      ...
      tableBody.push([
        ...
        { text: \`\${empresaName}\\n\${l.name}\`, style: 'tableCell', fontSize: 8, fillColor },
        { text: lastVisitLabel, style: 'tableCell', fontSize: 8, fillColor },
        { text: mortPercent, style: 'tableCell', fontSize: 8, alignment: 'center', color: mortColor, bold: mortPercentNum > 3.0, fillColor },
        { text: (consumoRealCab !== undefined && consumoEsperado !== null) ? \`\${consumoRealCab.toFixed(1)} / \${consumoEsperado.toFixed(1)}\` : '-', style: 'tableCell', fontSize: 8, alignment: 'center', fillColor },
