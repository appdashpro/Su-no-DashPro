      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col min-h-[450px]">
          <div className="mb-6">
            ...
            <p className="text-xs text-slate-500 mt-1">Correlação na última avaliação dos lotes</p>
          </div>
          <div className="flex-1 min-h-[350px]">
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: -20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                <XAxis type="number" dataKey="sanidade" name="Sanidade" unit="%" domain={[0, 100]} stroke="#94a3b8" fontSize={11} label={{ value: 'Índice de Sanidade (%)', position: 'bottom', offset: 0 }} tick={{fill: '#64748b'}} />
                <YAxis type="number" dataKey="mortalidade" name="Mortalidade" unit="%" stroke="#94a3b8" fontSize={11} label={{ value: 'Mortalidade (%)', angle: -90, position: 'insideLeft', offset: 25 }} tick={{fill: '#64748b'}} />
...
let fullContent = fs.readFileSync(file, 'utf-8');

// Replace everything from `return (` at line 483 to the end of the file
const parts = fullContent.split(' return (');
if (parts.length > 1) {
  // We keep everything before the last " return (" just in case there are multiple
  // But wait, there are returns inside map functions!
  // Let's use a regex that matches `return (\n <div className="space-y-4" ref={dashboardRef}>`
  const regex = /return \(\s*<div className="space-y-4" ref=\{dashboardRef\}>[\s\S]*\}\s*$/;
  fullContent = fullContent.replace(regex, renderCode);
  fs.writeFileSync(file, fullContent);
} else {
  console.error("Could not find return statement to replace");
}
