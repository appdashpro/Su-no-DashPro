...
export const getConsolidatedLotesTemplate = (
  ...
  selectedLotes.forEach(l => {
    ...
    if (evalSanitaria) {
      if (parseScore(evalSanitaria.granja?.ventilacao) > 0) validScores.push({ score: parseScore(evalSanitaria.granja?.ventilacao) });
      if (parseScore(evalSanitaria.granja?.limpeza_baias) > 0) validScores.push({ score: parseScore(evalSanitaria.granja?.limpeza_baias) });
      if (parseScore(evalSanitaria.granja?.qualidade_ar) > 0) validScores.push({ score: parseScore(evalSanitaria.granja?.qualidade_ar) });
      if (parseScore(evalSanitaria.qualidade_agua?.limpeza_caixa) > 0) validScores.push({ score: parseScore(evalSanitaria.qualidade_agua?.limpeza_caixa) });
  ...
  // Lotes ranking body - We need to expand it with new columns
  const tableBody: TableCell[][] = [
    [
      ...
      { text: 'Consumo / Meta (kg)', style: 'tableHeader', alignment: 'center' },
      { text: 'Aderência', style: 'tableHeader', alignment: 'center' },
      { text: 'Tratamentos', style: 'tableHeader', alignment: 'center' },
  ...
  if (selectedLotes.length > 0) {
    selectedLotes.forEach((l, index) => {
      ...
      const lastVisitLabel = lastVisit 
        ? \`\${formatDate(lastVisit.date)} (Idade: \${targetAge}d)\nTec: \${lastVisit.colaborador || '-'}\`
        : '-';
      ...
      tableBody.push([
        ...
        { text: \`\${empresaName}\n\${l.name}\`, style: 'tableCell', fillColor },
        { text: lastVisitLabel, style: 'tableCell', fillColor },
        { text: mortPercent, style: 'tableCell', alignment: 'center', fillColor },
        { text: (consumoRealCab !== undefined && consumoEsperado !== null) ? \`\${consumoRealCab.toFixed(1)} / \${consumoEsperado.toFixed(1)}\` : '-', style: 'tableCell', alignment: 'center', fillColor },
        { text: curveAccuracy !== null ? \`\${Math.round(curveAccuracy)}%\` : '-', style: 'tableCell', alignment: 'center', fillColor },
        { text: hasTreatments, style: 'tableCell', alignment: 'center', bold: hasTreatments === 'Sim', color: treatmentsColor, fillColor },
        { text: sanitStatus, style: 'tableCell', color: sanitColor, alignment: 'center', bold: true, fillColor }
      ]);
    });
  } else {
    tableBody.push([
