              ...
              <div className="bg-slate-50/50 p-6 rounded-xl border border-slate-200/80">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Cadastre as versões históricas para os cálculos de metas</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                ...
                {isAddingCurva && (
                  <div className="bg-white p-4 rounded-lg border border-emerald-200 shadow-sm mb-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-md text-sm focus:ring-1 focus:ring-emerald-500 outline-none"
                        />
                      </div>
                      <div>
                    ...
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        onClick={() => {
                          ...
                          setCurvasConfig([...curvasConfig, nova]);
                          setIsAddingCurva(false);
                          setNewCurva({});
                        }}
                        className="px-4 py-2 text-sm bg-emerald-600 text-white hover:bg-emerald-700 rounded-md font-medium"
                      >
                        Adicionar Curva
                      </button>
                    </div>
                              ...
                              <button
                                onClick={() => {
                                  if(confirm('Tem certeza que deseja remover esta curva? Lotes que dependem dela podem ser afetados.')) {
                                    setCurvasConfig(curvasConfig.filter(cv => cv.id !== c.id));
                                  }
                                }}
